...
class mav_viewer():
    def __init__(self, parent=None):
        ...
        self.window.setWindowTitle('Aircraft Viewer')
    ...
    def _Euler2Rotation(self, phi, theta, psi):
        ...
        c_phi = np.cos(phi) #roll
        s_phi = np.sin(phi) 
        c_theta = np.cos(theta) #pitch
        s_theta = np.sin(theta)
        c_psi = np.cos(psi) #yaw
        s_psi = np.sin(psi)
        ...
        R = R_roll @ R_pitch @ R_yaw  # vehicle (with inertia vectors) to body (Equation 2.4 in book)
